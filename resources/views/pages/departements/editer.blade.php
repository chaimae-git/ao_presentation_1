                    ...
                    <div class="form py-5">
                        <div class="form-group mb-3">
                            ...
                            <select name="bu_id" class="form-control">
                                ...
                                @foreach($bus as $bu)
                                    <option value="{{$bu->id}}" @if((old('bu_id')) && old('bu_id') == $bu->id) {{'selected'}}@elseif(($departement->bu_id) && $departement->bu_id == $bu->id) {{'selected'}} @endif>{{$bu->nom}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control" name="nom" placeholder="Nom" value={{old('nom', $departement->nom)}}>
                            @error('nom') <span class="text-danger">{{$message}}</span> @enderror
                        </div>
                        <div class="form-group mb-3">
                            <label for="nom">Description</label>
                            <textarea name="description" cols="10" class="form-control" placeholder="description">{{old('description', $departement->description)}}</textarea>
